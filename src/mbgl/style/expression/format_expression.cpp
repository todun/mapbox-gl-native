...
ParseResult FormatExpression::parse(const Convertible& value, ParsingContext& ctx) {
    ...
    for (std::size_t i = 1; i < argsLength - 1; i += 2) {
        ...
        const optional<Convertible> sectionIDOption = objectMember(options, kFormattedSectionSectionId);
        ParseResult sectionID;
        if (sectionIDOption) {
            sectionID = ctx.parse(*sectionIDOption, 1, {type::String});
            if (!sectionID) {
                return ParseResult();
            }
        }
...
mbgl::Value FormatExpression::serialize() const {
    ...
    for (const auto& section : sections) {
        ...
        if (section.sectionID) {
            options.emplace(kFormattedSectionSectionId, (*section.sectionID)->serialize());
        }
...
EvaluationResult FormatExpression::evaluate(const EvaluationContext& params) const {
    ...
    for (const auto& section : sections) {
        ...
        optional<std::string> evaluatedSectionID;
        if (section.sectionID) {
            auto sectionIDResult = (*section.sectionID)->evaluate(params);
            if (!sectionIDResult) {
                return sectionIDResult.error();
            }
            auto sectionIDValue = ValueConverter<std::string>::fromExpressionValue(*sectionIDResult);
            if (!sectionIDValue) {
                return EvaluationError { "Format section id option must evaluate to string" };
            }
            evaluatedSectionID = *sectionIDValue;
        }
        evaluatedSections.emplace_back(*evaluatedText, evaluatedFontScale, evaluatedTextFont, evaluatedSectionID);
    }
