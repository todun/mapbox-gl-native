...
const char* const kFormattedSectionSectionId = "id";
...
bool Formatted::operator==(const Formatted& other) const {
    ...
    for (std::size_t i = 0; i < sections.size(); i++) {
        ...
        if (thisSection.text != otherSection.text ||
            ...
            thisSection.sectionID != otherSection.sectionID) {
            return false;
        }
    }
    return true;
    ...
    if (isArray(value)) {
        ...
        for (std::size_t i = 0; i < arrayLength(value); ++i) {
            ...
            if (sectionLength > 1) {
                ...
                optional<Convertible> sectionIDMember = objectMember(sectionParams, kFormattedSectionSectionId);
                if (sectionIDMember) {
                    sectionID = toString(*sectionIDMember);
                    if (!sectionID) {
                        error.message = "Section id has to be a string.";
                        return nullopt;
                    }
                }
            }

            sections.push_back(FormattedSection(*sectionText, fontScale, textFont, sectionID));
        }
